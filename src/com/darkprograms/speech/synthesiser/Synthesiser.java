...
public class Synthesiser {
    ...
    public static void saveToFile(String fileName, InputStream in) {
        try(
            ...
            Logger.getLogger(Synthesiser.class.getName()).log(Level.SEVERE, null, ex);
        } catch (IOException ex) {
            Logger.getLogger(Synthesiser.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

    public static LinkedList<float[]> detectSoundIntervals(String audioFileName) throws IOException {
        //String audioFileName = "/home/eduardo/NetBeansProjects/JavaSpeechApiMaster/out.flac";

        ProcessBuilder builder = new ProcessBuilder(
            "/home/eduardo/bin/ffmpeg",
            "-i", audioFileName,
        ...
        LinkedList<float[]> cuts = new LinkedList<>();
        float duration = 0;
        float[] cut = new float[2];
        cut[0] = 0;

        while((inputLine = processReader.readLine()) != null) {
            System.out.println(inputLine);

            if(inputLine.startsWith("[silencedetect")) {
                if(inputLine.contains("silence_start")) {
                    if(cut == null) {
                    ...
                    cut[1] = Float.parseFloat(split[split.length-1]) + 0.5f;
                    cuts.add(cut);
                    cut = null;
                }
                else if(inputLine.contains("silence_end")) {
                    if(cut != null) {
                        System.out.print("ERROR");
                        break;
                    }
                    ...
                    cut[0] = Float.parseFloat(split[split.length-4]);
                }
            }
            else if(inputLine.contains("Duration:")) {
                String[] split = inputLine.trim().split(" ");
                String[] timeString = split[1].split(":");

                int hours = Integer.parseInt(timeString[0]);
                int minutes = Integer.parseInt(timeString[1]);
                float seconds = Float.parseFloat(timeString[2].substring(0, timeString[2].length()-1));
